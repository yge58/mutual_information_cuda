...
bool itkImageMutualInformation( const std::string &image1Name, const std::string &image2Name, const uint &bin_count )
{
  ...
  histogramFilter->SetMarginalScale( 0.1 );
  // Software Guide : EndCodeSnippet
  ...
  size[0] = bin_count - 1;  // number of bins for the first  channel
  size[1] = bin_count - 1;  // number of bins for the second channel
  ...
  while( itr != end )
    ...
    if( count > 0.0 )
      ...
      double Entropy = - probability * std::log( probability ) / std::log( 2.0 );
      JointEntropy += Entropy;
      // std::cout << "entropy = " << Entropy << "\tcount = " << count << std::endl;
      }
    ++itr;
    }
  // Software Guide : EndCodeSnippet
  ...
  std::cout << "\nITK Entropy Calculation Time = " << clock.GetMean() * 1000 << " ms" << std::endl << std::endl;
